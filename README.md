# Cloud Architecture Portfolio

## 👋 About Me
I am a Microsoft 365 & Azure Architect with 10+ years of experience designing secure, scalable cloud solutions
across enterprise and public-sector environments.

## 🔐 Focus Areas
- Microsoft Entra ID (Azure AD) & Zero Trust Architecture
- Microsoft 365 Security & Compliance
- Windows 365 Cloud PC
- Intune & Endpoint Security
- Identity Governance & Conditional Access

## 📄 Publications
- Cloud PC Transformation with Windows 365 (Zenodo)
- Zero Trust Security Framework using Microsoft Entra ID
- Microsoft Entra ID as a Cloud-First Identity Control Plane
  
## 🧩 Architecture Assets
- Reference diagrams
- Deployment guides
- Security design patterns

## 📫 Connect
- LinkedIn: https://linkedin.com/in/dilawarshaikh
- GitHub: https://github.com/dilawarhassanshaikh
- Gmail: dilawarshaikh0@gmail.com
